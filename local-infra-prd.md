...
| 비밀 저장 | 상태 디렉터리의 로컬 금고(`machine.key` + `keychain.vault`, 0600, AES-256-GCM) | macOS·Linux 동일. OS Keychain/Secret Service는 사용하지 않음(세션 없는 환경에서 불안정). passphrase 보호가 필요하면 `file` 모드 |
...
| TAR-007 | P0 | 비밀 값은 상태 디렉터리의 `0600` 로컬 금고(`machine.key` + `keychain.vault`)에 저장한다. OS Keyring/Secret Service는 사용하지 않는다. |
...
- DB 비밀번호와 버킷 시크릿 키는 상태 디렉터리의 `0600` 로컬 금고에 저장한다. 어느 플랫폼에서든 같은 방식이다.
- 금고를 열 수 없을 때는 비밀 값을 저장하지 않는 제한 모드로 내려가고, 선택적으로 사용자 암호로 암호화한 파일 저장(`file` 모드)을 허용한다(§19.8).
...
| 상태 디렉터리에 금고를 만들 수 없음 | 비밀 저장 실패 | 비저장 제한 모드, 암호화 파일 옵션. 헤드리스 자체는 문제가 되지 않는다 |
...
| 8 | 헤드리스 서버에서 Keyring 대체 저장 방식 | 기본 `keyring` 모드가 모든 플랫폼에서 상태 디렉터리의 로컬 금고(`machine.key` + `keychain.vault`, `0600`)를 쓰므로 헤드리스에서도 저장된다. 선택적으로 암호화 파일 모드(AES-256-GCM, 암호에서 유도한 키, `0600`). 금고를 열 수 없을 때만 비저장 제한 모드. | §11.1. 평문을 조용히 기록하지 않는다. 암호는 `LINF_PASSPHRASE`로 받는다. |
